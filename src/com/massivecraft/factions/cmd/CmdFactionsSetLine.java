...
public class CmdFactionsSetLine extends CmdFactionsSetX {
	...
	public static final BlockFace[] axis = { BlockFace.SOUTH, BlockFace.WEST,
	...
	public Integer getRadius() throws MassiveException {
		...
		if (radius > MConf.get().setLineMax && !msender.isOverriding()) {
			msg("<b>The maximum amount allowed is <h>%s<b>.",
					MConf.get().setLineMax);
			return null;
		}
	...
	public BlockFace getDirection() throws MassiveException {
		...
		if (direction == null) {
			...
			msender.msg(ChatColor.LIGHT_PURPLE + direction + ChatColor.RED
	...
	public Set<PS> getChunks() throws MassiveException {
		...
		for (int i = 0; i < getRadius() - 1; i++) {
			location = location.add(getDirection().getModX() * 16, 0,
					getDirection().getModZ() * 16);
